...
router.get('/admin/staff', requireAuth(['admin']), asyncRoute(async (_req, res) => {
  const result = await query(`select u.id,u.name,u.email,u.created_at,c.id as counter_id,c.name as counter_name,c.branch_id,coalesce(json_agg(json_build_object('id',s.id,'name',s.name)) filter(where s.id is not null),'[]'::json) as services from public.users u left join public.counters c on c.staff_id=u.id left join public.staff_counter_services scs on scs.staff_id=u.id and scs.counter_id=c.id left join public.services s on s.id=scs.service_id where u.role='staff' group by u.id,c.id order by u.name`);
  ok(res, result.rows);
}));
