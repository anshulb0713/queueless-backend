...
import { NotificationType, notifyThreeAhead, sendTokenNotification } from '../services/notification.service.js';

const id = z.string().uuid();
const tokenInput = z.object({ branchId: id, serviceId: id, fcmToken: z.string().trim().min(20).max(4096) });
...
router.post('/tokens', requireCustomerAuth, asyncRoute(async (req, res) => {
  ...
  const token = await transaction(async client => {
    await client.query(`update public.users set fcm_token=$2, fcm_token_updated_at=now() where id=$1`, [customer.id, input.fcmToken]);
  ...
  await sendTokenNotification(token.id, 'joined');
...
router.patch('/tokens/:tokenId/cancel',requireCustomerAuth,asyncRoute(async(req,res)=>{const tokenId=id.parse(req.params.tokenId);const updated=await transaction(async client=>{const found=await client.query<{id:string;status:TokenStatus;service_id:string}>('select id,status,service_id from public.tokens where id=$1 and customer_id=$2 for update',[tokenId,req.customer!.id]);const token=found.rows[0];if(!token)throw new ApiError(404,'TOKEN_NOT_FOUND','Token not found');assertTransition(token.status,'cancelled');const result=await client.query(`update public.tokens set status='cancelled', cancelled_at=now() where id=$1 returning *`,[token.id]);await recalculateQueue(client,token.service_id);return result.rows[0];});await sendTokenNotification(updated.id,'cancelled');await notifyThreeAhead(updated.service_id);ok(res,updated,'Token cancelled');}));

router.get('/counters',requireAuth(),asyncRoute(async(req,res)=>{const branchId=req.query.branchId?id.parse(req.query.branchId):undefined;const result=await query(`select c.*,u.name as staff_name,t.token_number as current_token from public.counters c left join public.users u on u.id=c.staff_id left join public.tokens t on t.counter_id=c.id and t.status in ('called','serving') where ($1::uuid is null or c.branch_id=$1) and ($2::public.user_role='admin' or c.staff_id=$3) order by c.name`,[branchId??null,req.user!.role,req.user!.id]);ok(res,result.rows);}));
router.post('/counters',requireAuth(['admin']),asyncRoute(async(req,res)=>{const input=json(z.object({branchId:id,name:z.string().trim().min(2).max(100),staffId:id.optional() }),req.body);await assertStaff(input.staffId);const result=await query(`insert into public.counters(branch_id,name,staff_id) values($1,$2,$3) returning *`,[input.branchId,input.name,input.staffId??null]);ok(res,result.rows[0],'Counter created',201);}));
