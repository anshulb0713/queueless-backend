...
import { requireAuth, requireCustomerAuth, signToken, verifyPassword } from '../middlewares/auth.middleware.js';
...
const tokenInput = z.object({ branchId: id, serviceId: id });
...
router.patch('/tokens/:tokenId/cancel',requireCustomerAuth,asyncRoute(async(req,res)=>{const tokenId=id.parse(req.params.tokenId);const updated=await transaction(async client=>{const found=await client.query<{id:string;status:TokenStatus;service_id:string}>('select id,status,service_id from public.tokens where id=$1 and customer_id=$2 for update',[tokenId,req.customer!.id]);const token=found.rows[0];if(!token)throw new ApiError(404,'TOKEN_NOT_FOUND','Token not found');assertTransition(token.status,'cancelled');const result=await client.query(`update public.tokens set status='cancelled', cancelled_at=now() where id=$1 returning *`,[token.id]);await recalculateQueue(client,token.service_id);return result.rows[0];});ok(res,updated,'Token cancelled');}));
