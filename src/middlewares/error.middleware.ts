...
// Every asynchronous API controller uses this wrapper, so errors from validation,
// database queries, and notification work consistently reach the JSON error handler.
export const asyncRoute = (fn: (req: Request, res: Response, next: NextFunction) => Promise<unknown>) => async (req: Request, res: Response, next: NextFunction) => {
  try {
