...
export const openapiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      TokenRequest: { type: 'object', required: ['branchId', 'serviceId'], properties: { branchId: { type: 'string', format: 'uuid' }, serviceId: { type: 'string', format: 'uuid', description: 'Only one unresolved token is allowed per customer for this service. Tokens for other services remain allowed.' }, fcmToken: { type: 'string', nullable: true, description: 'Optional FCM registration token saved before the token is created.' } } },
      CategoryRequest: { type: 'object', required: ['name'], properties: { name: { type: 'string', example: 'Hospital' }, status: { type: 'string', enum: ['active', 'inactive'] } } },
      CounterReference: { type: 'object', required: ['counterId'], properties: { counterId: { type: 'string', format: 'uuid' } } },
      BranchRequest: { type: 'object', required: ['name', 'address', 'categoryId'], properties: { name: { type: 'string', example: 'City Care Clinic' }, address: { type: 'string', example: 'Ahmedabad' }, categoryId: { type: 'string', format: 'uuid' }, status: { type: 'string', enum: ['open', 'closed'] } } },
      ...
      StaffRequest: { type: 'object', required: ['name', 'email', 'password'], properties: { name: { type: 'string', example: 'Alex Staff' }, email: { type: 'string', format: 'email' }, password: { type: 'string', format: 'password', minLength: 8 }, counterId: { type: 'string', format: 'uuid', description: 'Optional. When set, serviceIds is required.' }, serviceIds: { type: 'array', minItems: 1, items: { type: 'string', format: 'uuid' }, description: 'Selected active services at the assigned counter. Required when counterId is set.' } } },
  ...
  paths: {
    ...
    '/categories': { get: operation('List active branch categories for customer filtering', ['Branch and services']) },
    '/admin/categories': { get: operation('List every category and its branch count', ['Admin', 'Branch and services'], { security: adminAuth }), post: operation('Create a branch category', ['Admin', 'Branch and services'], { security: adminAuth, requestBody: body({ $ref: '#/components/schemas/CategoryRequest' }), successCode: 201, successDescription: 'Category created' }) },
    '/admin/categories/{categoryId}': { put: operation('Rename or turn a category on/off', ['Admin', 'Branch and services'], { security: adminAuth, description: 'Turning a category off requires no active queue tokens. It closes its branches and counters, inactivates its services, and disables assigned staff accounts. Turning it back on re-enables those staff accounts; child records stay closed/inactive until an admin reopens them.', parameters: [uuidParameter('categoryId', 'Category UUID')], requestBody: body({ type: 'object', properties: { name: { type: 'string' }, status: { type: 'string', enum: ['active', 'inactive'] } }, minProperties: 1 }) }), delete: operation('Delete a category with no attached branches', ['Admin', 'Branch and services'], { security: adminAuth, parameters: [uuidParameter('categoryId', 'Category UUID')] }) },
    ...
    '/admin/staff': { get: operation('List staff accounts and their counter assignments', ['Admin'], { security: adminAuth, parameters: [{ name: 'branchId', in: 'query', schema: { type: 'string', format: 'uuid' }, description: 'Optional. Limits results to staff assigned to counters in this branch.' }] }), post: operation('Create a staff account', ['Admin'], { security: adminAuth, requestBody: body({ $ref: '#/components/schemas/StaffRequest' }), successCode: 201, successDescription: 'Staff user created' }) },
    ...
    '/admin/staff/{staffId}/status': { patch: operation('Activate or deactivate a staff account', ['Admin'], { security: adminAuth, parameters: [staffId], requestBody: body({ type: 'object', required: ['isActive'], properties: { isActive: { type: 'boolean', description: 'False prevents this staff user from logging in or operating their counter.' } } }) }) },
    ...
    '/branches/{branchId}': { get: operation('Get a branch', ['Branch and services'], { parameters: [branchId] }), put: operation('Update branch details, category, or turn the branch on/off', ['Admin', 'Branch and services'], { security: adminAuth, parameters: [branchId], requestBody: body({ type: 'object', properties: { name: { type: 'string' }, address: { type: 'string' }, categoryId: { type: 'string', format: 'uuid' }, status: { type: 'string', enum: ['open', 'closed'] } }, minProperties: 1 }) }), delete: operation('Delete an empty branch and its empty child services/counters', ['Admin', 'Branch and services'], { security: adminAuth, parameters: [branchId] }) },
    ...
    '/tokens/history': { get: operation('List all customer tokens, including cancelled and completed history', ['Customer'], { security: customerAuth, parameters: [{ name: 'branchId', in: 'query', schema: { type: 'string', format: 'uuid' } }, { name: 'limit', in: 'query', schema: { type: 'integer', minimum: 1, maximum: 100, default: 50 } }] }) },
