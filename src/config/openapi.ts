...
export const openapiDocument = {
  ...
  components: {
    ...
    schemas: {
      ...
      CounterRequest: { type: 'object', required: ['branchId', 'name'], properties: { branchId: { type: 'string', format: 'uuid' }, name: { type: 'string', example: 'Counter 3' }, staffId: { type: 'string', format: 'uuid' }, serviceIds: { type: 'array', items: { type: 'string', format: 'uuid' } } } },
  ...
  paths: {
    ...
    '/branches': { get: operation('List branches with waiting counts and estimated wait time', ['Branch and services']) },
    '/branches/{branchId}': { get: operation('Get a branch', ['Branch and services'], { parameters: [branchId] }) },
    ...
    '/services/{serviceId}': { put: operation('Update a service name, prefix, duration, or status', ['Admin', 'Branch and services'], { security: adminAuth, parameters: [serviceId], requestBody: body({ type: 'object', properties: { name: { type: 'string' }, prefix: { type: 'string' }, averageDuration: { type: 'integer', minimum: 1, maximum: 240 }, status: { type: 'string', enum: ['active', 'inactive'] } }, minProperties: 1 }) }) },
